...
| **CORS restringido.** Solo los orígenes de `GYNFEM_CORS_ORIGINS`, nunca `*`. En `development` y `test`, solo localhost. En `production`, solo `https` y nunca localhost. Sin credenciales | `tests/api/test_api_cors.py`; `test_comodin_se_rechaza`; `test_en_produccion_se_exige_https_y_no_localhost` |
| **Logs sin datos clínicos ni identificadores.** Una línea JSON por evento, con `request_id`. Solo claves de una lista cerrada. Se registra la plantilla de la ruta, nunca el path real, la query string, las cabeceras ni el cuerpo. De una excepción se registra el tipo y la pila, nunca su mensaje. El log de acceso de uvicorn se desactiva (`--no-access-log`) | `tests/api/test_api_logging.py` (`test_logs_no_contienen_valores_clinicos`, con un valor centinela en path, query, cabecera, cuerpo y mensaje de excepción) |
...
| **`X-Request-ID` saneado.** Uno entrante se respeta solo si tiene de 1 a 64 caracteres `[A-Za-z0-9-]`; si no, se genera otro, para que no se puedan inyectar líneas en los logs | `test_request_id_malicioso_se_reemplaza` |
